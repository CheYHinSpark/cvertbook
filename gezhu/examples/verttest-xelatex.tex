...
\makeatletter
\AddToShipoutPicture{%
\setlength{\@tempdima}{\paperwidth - \myleft - \myright}
\setlength{\@tempdimb}{\paperheight - \mytop}
\loop
  \put(\strip@pt\myleft,\strip@pt\@tempdimb){%
  \framebox(\strip@pt\@tempdima,\strip@pt\mylineskip){}
  }
\ifdim \@tempdimb > \mybottom
  \advance\@tempdimb by -\mylineskip
\repeat
}
\makeatother
...
%纸张横排，因为后面会转动90度，实际效果就是变回了竖着的纸张
\usepackage[landscape]{geometry} 
\geometry{left=\myleft,
  right=\myright,
  top=\mytop,
  bottom=\mybottom}
...
%zhspacing命令，分别是中文基本汉字和标点符号
\newfontfamily\myzhfont[RawFeature={vertical:}]{SimSun}
%\newfontfamily\myzhfont[RawFeature={vertical:}]{HanyiSentyYongleEncyclopedia.ttf}
\def\zhfont{\myzhfont}%感觉这样可以避免问题。。
\newfontfamily\myzhpunctfont[RawFeature={vertical:+vert:+vhal}]{SimSun}
\def\zhpunctfont{\myzhpunctfont}
\zhspacing
\haltskipscheme
